...
int storage_socketID[MAX_SOCKET] = {0};
char message[20];
int32_t x = 0, var = 0;
...
void *SendMessages (void *ptr)
{
    ...
    while (1)
    {
        ...
        for (x = 0; x < 10; x++)
        {
			...
			if (storage_socketID[x] != 0)
			{
				ID = recv(storage_socketID[x], buffer, 1024, MSG_NOSIGNAL);
				printf("socket: %d\n",storage_socketID[x]);
				printf("FD: %d\n",ID);
		...
		{//Verificación CS
			...
			if(buffer[0] == 0xAA)//Verificación de SOF
			{
				...
				{
					if(buffer[2] == 2)
					{//Lectura de registro pedido por el cliente
...
{	 
    ...
    int32_t server_fd, iWriteHistRc; 
	...
	{  
		printf("Accept\n");
		newScoketConnectionFD = accept(server_fd, (struct sockaddr *)&address,(socklen_t*)&addrlen);
	...
	{	
      ...
      else
      {
        continue;
			...
			printf("adding new connection listSocket[%d] = %d\n", x, storage_socketID[x]);
			
			iWriteHistRc = send(storage_socketID[x], message, strlen(message), MSG_NOSIGNAL);

			if (iWriteHistRc <= 0)
			{
				printf("Shuting down a socket of index: %d\n", x);
				shutdown(storage_socketID[x], SHUT_RDWR);
				storage_socketID[x] = 0;				
			}
			break;	  
	}	
